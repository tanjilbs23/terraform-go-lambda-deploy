...
type Response struct {
	VippsRedirectURL string `json:"vippsRedirectURL"`
}
...
func main() {
	...
	accessTokenResponse, _ = getAccessToken()
...
func Handler(req json.RawMessage) (Response, error) {
	...
	if payloadErr != nil {
		log.Fatalf("req data unmarshal err: %+v", reqData)
		return Response{}, payloadErr
	}

	trip, tripErr := getTripData(reqData)
	if tripErr != nil {
		return Response{}, tripErr
	}
...
func getTripData(payload Event) (Trip, error) {
	...
	if err != nil {
		Error = err
		log.Fatalf("trip db get query err: %s", Error.Error())
	}
...
func updateTripData(reqData Event, newAvailableRegularTickets int) (bool, error) {
	...
	if err != nil {
		Error = err
		log.Fatalf("trip db update query err: %s", Error.Error())
		return false, Error
	}
...
func initPayment(reqData Event, trip Trip) (Response, error) {
	...
	newPayload.MerchantInfo.FallBack = vippsConfig.Fallback + "?trip_id=" + trip.Id
	...
	newPayload.Transaction.Amount = trip.ShareleadTotalPayableAmount * 100 //Convert krona to öre
